  ...
  try {
    ...
    const summary = regions.map((region) => {
      ...
      return {
        ...
        apartments: regionItems.slice(0, 50),
      };
    });
...
app.get("/api/apartments", async (req, res) => {
  try {
    const lawdCd = req.query.lawdCd || "11680";
    const dealYmd = req.query.dealYmd || "202405";
    ...
    const data = await fetchApartmentData(region, dealYmd);
    await saveApartmentsToDB(data, lawdCd, dealYmd);
